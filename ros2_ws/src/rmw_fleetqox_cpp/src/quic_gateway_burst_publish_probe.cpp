...
int positive_int_env(const char * name, int fallback, int max_value)
{
...
int main()
{
  const int burst_count = positive_int_env("FLEETQOX_RMW_QUIC_GATEWAY_BURST_COUNT", 4, 32);
  ...
  rmw_node_t * node = rmw_create_node(&context, "fleetqox_quic_gateway_burst_probe", "/fleetqox");
  if (node == nullptr) {
  ...
  const char * topic = "/fleetqox/quic_gateway_burst_publish_probe";
  ...
  for (int index = 0; index < burst_count; ++index) {
    ...
    const std::string payload =
      "fleetqox quic gateway async burst publish " + std::to_string(index);
  ...
  for (int attempt = 0; attempt < 240; ++attempt) {
    ...
    if (quic_frames_sent >= static_cast<std::uint64_t>(publish_ok_count) &&
  ...
  const bool ok = publish_ok_count == burst_count &&
                  publish_failed_count == 0 &&
                  quic_frames_sent >= static_cast<std::uint64_t>(burst_count) &&
                  quic_bytes_sent > 0 &&
                  (!quic_async_enabled || quic_frames_enqueued >=
                  static_cast<std::uint64_t>(burst_count)) &&
                  quic_frames_failed == 0 &&
                  quic_frames_dropped == 0 &&
                  quic_queue_depth == 0 &&
                  quic_last_exit_code == 0 &&
  ...
  std::cout << "{\"schema_version\":\"fleetrmw.quic_gateway_burst_publish_probe.v1\",";
  std::cout << "\"status\":\"" << (ok ? "ok" : "failed") << "\",";
  std::cout << "\"topic\":\"" << topic << "\",";
  std::cout << "\"burst_count\":" << burst_count << ",";
  std::cout << "\"publish_ok_count\":" << publish_ok_count << ",";
  std::cout << "\"publish_failed_count\":" << publish_failed_count << ",";
  ...
  std::cout << "\"publish_returned_after_enqueue\":" <<
    (quic_async_enabled && quic_frames_enqueued >= static_cast<std::uint64_t>(burst_count) ?
    "true" : "false") << ",";
  std::cout << "\"rmw_publish_path_integrated\":true}" << std::endl;
