#!/usr/bin/env python3
"""Validate QoS/QoE admission and repair coupling over QUIC/H3."""
...
SCHEMA_VERSION = "fleetrmw.docker_quic_qox_repair_probe.v1"
PROBE_SCHEMA_VERSION = "fleetrmw.quic_qox_repair_probe.v1"
...
def probe_ok(row: dict[str, Any]) -> bool:
    return (
        ...
        and row.get("low_score_rejected") is True
        and row.get("high_score_admitted") is True
        and row.get("repair_scheduler_admitted") is True
        and row.get("repair_capacity_deferred") is True
        ...
        and row.get("qos_qoe_admission_repair_coupling_claim") is True
...
def service_ok(row: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and metrics.get("post_requests") == 4
...
def run_case(
    *,
    ...
    service_name = f"fleetrmw-qox-repair-service-{suffix}"
    ...
    try:
        if ready:
            uri = (
                "https://localhost:4499/fleetrmw/v1/frames?domain_id=42&"
                "topic=%2Ffleetqox%2Fqox_repair&consumer_id=qox-consumer"
            )
            command = (
                ...
                "export FLEETQOX_RMW_QUIC_GATEWAY=fleetqox-admission-gateway:4499 && "
                ...
                "fleetrmw_quic_qox_repair_probe"
            )
            client = run(
                [
                    "docker",
                    ...
                    "--rm",
                    "--name",
                    f"fleetrmw-qox-repair-client-{suffix}",
                    "--network",
                    network,
                    "--cap-add",
                    "NET_ADMIN",
...
def run_probe(
    *, root: Path, image: str, iterations: int, keep_temp: bool
    ...
    temp_root = root / f".tmp_fleetrmw_quic_qox_repair_{os.getpid()}"
    certs = temp_root / "certs"
        ...
        json.dumps(
            {
                ...
                "rules": [
                    {
                        "domain_id": 42,
                        "topic": "/fleetqox/qox_repair",
                        "traffic_class": "control",
                        "max_accepted_frames": 1,
                        "min_admission_score": 0.5,
                        "allowed_publishers": ["qox-publisher"],
                    }
                ],
                "repair": {
                    "capacity_bytes": 1024,
    ...
    build_root = "/work/.tmp_fleetrmw_quic_qox_repair_build"
    install = "/work/.tmp_fleetrmw_quic_qox_repair_install"
    log_root = "/work/.tmp_fleetrmw_quic_qox_repair_log"
    cert_result = run(
        [
            "docker",
            "run",
    ...
    network = f"fleetrmw-qox-repair-net-{os.getpid()}"
    ...
    return {
        ...
        "qos_qoe_metadata_wire_claim": status == "ok",
        "qos_qoe_admission_score_claim": status == "ok",
        "fleet_repair_scheduler_gateway_coupling_claim": status == "ok",
        "repair_capacity_defer_fail_closed_claim": status == "ok",
        "h3_connection_reuse_across_admission_operations_claim": status == "ok",
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        default="results_rmw_socket/docker_quic_qox_repair_probe_summary.json",
    )
    ...
    print("fleetrmw-quic-qox-repair-probe")
