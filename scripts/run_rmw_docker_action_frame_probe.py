"""Run a Docker action-frame contract probe against rmw_fleetqox_cpp."""
...
SCHEMA_VERSION = "fleetrmw.rmw_docker_action_frame_probe.v1"
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        default="results_rmw_socket/docker_rmw_action_frame_probe_summary.json",
    )
    ...
    if args.json:
        ...
        print("fleetrmw-rmw-action-frame-probe")
        print(f"  status: {summary['status']}")
        print(f"  roles: {summary.get('probe', {}).get('roles')}")
...
/tmp/fleetrmw_install/rmw_fleetqox_cpp/lib/rmw_fleetqox_cpp/fleetrmw_action_frame_probe \
  > /tmp/fleetrmw_action_frame_probe.out 2> /tmp/fleetrmw_action_frame_probe.err
probe_ret=$?
...
stdout = Path("/tmp/fleetrmw_action_frame_probe.out").read_text()
stderr = Path("/tmp/fleetrmw_action_frame_probe.err").read_text()
probe = {}
...
expected_roles = ["goal", "feedback", "status", "result", "cancel"]
summary = {
    "schema_version": "fleetrmw.rmw_docker_action_frame_probe.v1",
    "status": "pending",
    "probe": probe,
...
summary["status"] = "ok" if (
    ...
    probe.get("roles") == expected_roles and
    probe.get("not_expired") is True and
    probe.get("expired") is True and
    probe.get("rejects_service_schema") is True and
    stderr == ""
) else "failed"
print(json.dumps(summary, sort_keys=True))
PY
