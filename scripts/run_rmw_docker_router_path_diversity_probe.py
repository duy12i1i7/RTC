"""Run a Docker dual-router path-diversity reliability probe for rmw_fleetqox_cpp."""
...
SCHEMA_VERSION = "fleetrmw.rmw_router_path_diversity_probe.v1"
DEFAULT_IMAGE = "ros:jazzy-ros-base"
DEFAULT_TOPIC = "/fleetqox/router_path_diversity_probe"
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        default="results_rmw_socket/docker_router_path_diversity_probe_summary.json",
    )
    ...
    if args.json:
        ...
        print("fleetrmw-router-path-diversity-probe")
        print(f"  status: {summary['status']}")
        print(f"  primary_dropped: {summary.get('primary_router', {}).get('test_dropped_frames')}")
...
def run_probe(*, root: Path, image: str, topic: str) -> dict[str, Any]:
    ...
    network = f"fleetrmw-path-div-net-{suffix}"
    primary_router_name = f"fleetrmw-path-div-primary-{suffix}"
    backup_router_name = f"fleetrmw-path-div-backup-{suffix}"
    subscriber_name = f"fleetrmw-path-div-sub-{suffix}"
    build_base = "/work/.tmp_fleetrmw_router_path_diversity_build"
    install_base = "/work/.tmp_fleetrmw_router_path_diversity_install"
    log_base = "/work/.tmp_fleetrmw_router_path_diversity_log"
    ...
    try:
        ...
        start_container(
            ...
            command=(
                ...
                f"{router_binary} --bind 0.0.0.0:48360 "
        ...
        start_container(
            ...
            command=(
                f"source /opt/ros/jazzy/setup.bash && source {install_base}/setup.bash && "
                f"{router_binary} --bind 0.0.0.0:48361 "
                "--expected-frames 3 --expected-ack-nack-frames 3 "
                "--expected-route-advertisements 1 --expected-graph-advertisements 2 "
                "--timeout-ms 9000"
            ),
        ...
        start_container(
            ...
            command=(
                ...
                "FLEETQOX_RMW_BIND=0.0.0.0:48362 "
                f"FLEETQOX_RMW_PEERS={primary_router_name}:48360,{backup_router_name}:48361 "
        ...
        status = (
            ...
            primary_router_result.get("forwarded_frames", 0) >= 2 and
            backup_router_result.get("forwarded_frames", 0) >= 3 and
            primary_router_result.get("ack_nack_forwarded", 0) >= 3 and
            backup_router_result.get("ack_nack_forwarded", 0) >= 3 and
            publisher_result.get("ack_nack_received", 0) >= 3 and
