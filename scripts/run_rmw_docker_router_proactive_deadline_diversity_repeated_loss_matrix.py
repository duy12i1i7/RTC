"""Repeat proactive deadline diversity under stochastic netem loss."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            "results_rmw_socket/"
            "docker_router_proactive_deadline_diversity_repeated_loss_matrix_summary.json"
        ),
    )
    ...
    for loss_percent in loss_percents:
        for repetition_id in repetitions:
            result = run_probe(
                root=root,
                image=args.image,
                topic=f"{args.topic}/rep-{repetition_id}",
                deadline_ms=max(args.deadline_ms, 1),
                primary_profile=args.primary_profile,
                backup_profile=args.backup_profile,
                loss_percent=loss_percent,
            )
            telemetry = result.get("delivery_telemetry", [])
            rows.append({
                "repetition_id": repetition_id,
                "loss_percent": loss_percent,
                "status": result.get("status", "failed"),
                "evidence_ok": result.get("status") == "ok",
                "on_time_sequences": result.get("on_time_sequences", []),
                "max_latency_ms": max(
                    (float(row.get("latency_ms", 0.0)) for row in telemetry),
                    default=0.0,
                ),
                "adaptive_redundant_frames": result.get("publisher", {}).get(
                    "adaptive_redundant_frames", 0
                ),
                "nack_retransmissions": result.get("publisher", {}).get(
                    "nack_retransmissions", 0
                ),
                "primary_test_dropped_frames": result.get("primary_router", {}).get(
                    "test_dropped_frames", 0
                ),
    ...
    summary = {
        ...
        "total_redundant_frames": sum(
            int(row["adaptive_redundant_frames"]) for row in rows
        ),
        "total_nack_retransmissions": sum(
            int(row["nack_retransmissions"]) for row in rows
        ),
        "rows": rows,
    }
    ...
    if args.json:
        ...
        print("fleetrmw-router-proactive-deadline-diversity-repeated-loss-matrix")
