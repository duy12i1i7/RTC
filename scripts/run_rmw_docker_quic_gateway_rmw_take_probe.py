"""Verify rmw_take_serialized_message can fetch a FleetRMW frame over QUIC/TLS/H3.

This is an opt-in, on-demand RMW take-path proof.  It exercises
rmw_take_serialized_message -> QuicGatewayTransport::receive() -> ngtcp2/GnuTLS
gtlsclient --download -> FleetRMW data-frame decode/enqueue -> serialized take.
It is still subprocess-backed and is not a production full-duplex QUIC backend.
"""
...
SCHEMA_VERSION = "fleetrmw.docker_quic_gateway_rmw_take_probe.v1"
...
def expected_frame_bytes() -> bytes:
    payload = b"fleetqox-quic-gateway-rmw-take-v1"
    return (
        b"FRMW1\n"
        b'{"schema_version":"fleetrmw.data_frame.v1",'
        b'"kind":"sidecar_packet_frame",'
        b'"domain_id":0,'
        b'"route":{"robot_id":"robot_quic_rmw_take_0001",'
        b'"topic":"/fleetqox/quic_gateway_rmw_take_probe"},'
        b'"sample_envelope":{'
        b'"robot_id":"robot_quic_rmw_take_0001",'
        b'"topic":"/fleetqox/quic_gateway_rmw_take_probe",'
        b'"publisher_id":"fpub-quic-gateway-rmw-take-0001",'
        b'"source_sequence_number":31,'
        b'"source_timestamp_ns":31000000'
        b"},"
...
def run_probe(*, root: Path, image: str, port: int) -> dict[str, Any]:
    ...
    tmp = root / f".tmp_fleetrmw_quic_gateway_rmw_take_{suffix}"
    htdocs = tmp / "htdocs"
    qlogs = tmp / "qlogs"
    certs = tmp / "certs"
    ...
    try:
        ...
        command = (
            ...
            "--timeout=12s --handshake-timeout=5s --no-quic-dump --no-http-dump "
        ...
        ok = (
            ...
            and probe_json.get("rmw_take_path_integrated") is True
            and probe_json.get("quic_gateway_take_path_download") is True
            and probe_json.get("payload_ok") is True
        ...
        return {
            ...
            "transport": "ngtcp2_gtls_quic_tls_h3_get_gateway_rmw_take_path",
            ...
            "rmw_take_path_integrated": probe_json.get("rmw_take_path_integrated") is True,
            "take_path_scope": "rmw_take_serialized_message_on_demand_quic_gateway_get",
            "payload_ok": probe_json.get("payload_ok") is True,
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        default="results_rmw_socket/docker_quic_gateway_rmw_take_probe_summary.json",
    )
