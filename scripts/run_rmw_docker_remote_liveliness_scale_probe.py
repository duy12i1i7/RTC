"""Repeat 64-endpoint remote MANUAL_BY_TOPIC liveliness under Docker/netem."""
...
SCHEMA_VERSION = "fleetrmw.docker_remote_liveliness_scale_probe.v1"
...
def observer_ok(probe: dict[str, Any]) -> bool:
    return (
        probe.get("status") == "ok"
        and probe.get("remote_manual_64_endpoint_scale_claim") is True
        and probe.get("exact_aggregate_transition_claim") is True
        and probe.get("expiry_preserves_matching_claim") is True
        and probe.get("publisher_count") == 64
        and probe.get("kept_alive_publisher_count") == 32
        and probe.get("publishers_during_half_expiry") == 64
        and probe.get("publishers_after_remove") == 0
        and probe.get("connected_alive_change_sum") == 64
        and probe.get("half_expired_alive_change_sum") == -32
        and probe.get("half_expired_not_alive_change_sum") == 32
        and probe.get("reasserted_alive_change_sum") == 32
        and probe.get("all_expired_alive_change_sum") == -64
        and probe.get("all_expired_not_alive_change_sum") == 64
        and probe.get("removed_not_alive_change_sum") == -64
        and probe.get("matched_change_sum") == 64
        and probe.get("unmatched_change_sum") == -64
        and int(probe.get("assertions_received", 0)) >= 320
        and probe.get("manual_liveliness_expiries") == 96
        and probe.get("manual_liveliness_reassertions") == 32
        and int(probe.get("callback_events", 0)) > 0
...
def run_case(
    *, root: Path, image: str, install: str, network: str, index: int
    ...
    observer = f"fleetrmw-remote-live-scale-observer-{os.getpid()}-{index}"
    advertiser = f"fleetrmw-remote-live-scale-advertiser-{os.getpid()}-{index}"
    binary = (
        f"{install}/rmw_fleetqox_cpp/lib/rmw_fleetqox_cpp/"
        "fleetrmw_remote_liveliness_scale_probe"
    )
    common_env = (
    ...
    observer_command = (
        ...
        f"{common_env}FLEETQOX_RMW_BIND=0.0.0.0:48620 "
    ...
    try:
        if ready:
            advertiser_command = (
                f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
                "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
                f"{common_env}FLEETQOX_RMW_BIND=0.0.0.0:48621 "
                f"FLEETQOX_RMW_PEERS={observer}:48620 "
    ...
    advertiser_ok = (
        advertiser_probe.get("status") == "ok"
        and advertiser_probe.get("publisher_count") == 64
        and advertiser_probe.get("kept_alive_publisher_count") == 32
        and int(advertiser_probe.get("keepalive_assertions", 0)) >= 480
        and advertiser_probe.get("keepalive_ok") is True
        and advertiser_probe.get("second_half_reasserted") is True
...
def run_probe(
    *, root: Path, image: str, iterations: int, keep_temp: bool
    ...
    build_root = "/work/.tmp_fleetrmw_remote_liveliness_scale"
    ...
    network = f"fleetrmw-remote-live-scale-net-{os.getpid()}"
    ...
    return {
        ...
        "remote_liveliness_64_endpoint_scale_claim": ok,
        "remote_liveliness_exact_aggregate_transition_claim": ok,
        "remote_liveliness_scale_repeated_claim": ok and run_count >= 5,
        "publisher_count": last_observer.get("publisher_count"),
        "kept_alive_publisher_count": last_observer.get(
            "kept_alive_publisher_count"
        ),
        "publishers_during_half_expiry": last_observer.get(
            "publishers_during_half_expiry"
        ),
        "publishers_after_remove": last_observer.get("publishers_after_remove"),
        "assertions_received": last_observer.get("assertions_received"),
        "manual_liveliness_expiries": last_observer.get(
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            "results_rmw_socket/"
            "docker_remote_liveliness_scale_probe_summary.json"
        ),
    )
