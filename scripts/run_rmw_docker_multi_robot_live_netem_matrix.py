"""Run the Docker multi-robot live RMW probe across real tc-netem profiles."""
...
SCHEMA_VERSION = "fleetrmw.rmw_multi_robot_live_netem_matrix.v1"
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        type=Path,
        default=Path("results_rmw_socket/docker_multi_robot_live_netem_matrix_summary.json"),
    )
    parser.add_argument(
        "--markdown",
        type=Path,
        default=Path("results_rmw_socket/docker_multi_robot_live_netem_matrix_report.md"),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="multiplier applied to profile packet loss; keep 0 for deterministic smoke",
    )
    parser.add_argument(
        "--netem-drain-s",
        type=float,
    ...
    if args.json:
        ...
        print("fleetrmw-multi-robot-live-netem-matrix")
        print(f"  status: {result['status']}")
        print(f"  image: {result['image']}")
        print(f"  profiles: {','.join(result['profiles'])}")
        print(f"  netem_required: {result['netem_required']}")
        print(f"  netem_loss_scale: {result['netem_loss_scale']}")
        print(f"  netem_drain_s: {result['netem_drain_s']}")
        print(f"  seed_semantics: {result['seed_semantics']}")
...
def render_markdown(summary: dict[str, object]) -> str:
    markdown = render_base_markdown(summary)
    return markdown.replace(
        "# RMW Multi-Robot Live Telemetry Matrix V1",
        "# RMW Multi-Robot Live Netem Matrix V1",
        1,
    ).replace(
        "- The matrix varies router telemetry profiles while keeping the same two-topic fleet workload.",
        "- The matrix varies router telemetry profiles and applies matching `tc netem` shaping on router containers.",
        1,
    )
