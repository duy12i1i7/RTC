...
def run_probe(*, root: Path, image: str, port: int) -> dict[str, Any]:
    ...
    qlogs = tmp / "qlogs"
    certs = tmp / "certs"
    build_base = root / ".tmp_fleetrmw_quic_gateway_take_build"
    install_base = root / ".tmp_fleetrmw_quic_gateway_take_install"
    log_base = root / ".tmp_fleetrmw_quic_gateway_take_log"
    probe = (
        "/work/.tmp_fleetrmw_quic_gateway_take_install/rmw_fleetqox_cpp/lib/"
