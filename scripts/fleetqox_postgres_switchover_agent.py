#!/usr/bin/env python3
"""DCS-authorized graceful Docker stop agent for PostgreSQL failback."""
...
SCHEMA_VERSION = "fleetrmw.postgresql_switchover_agent.v1"
...
class SwitchoverServer(ThreadingHTTPServer):
    dcs: EtcdQuorumLease
    lease_key: str
...
class SwitchoverHandler(BaseHTTPRequestHandler):
    server: SwitchoverServer
    ...
    def do_POST(self) -> None:
        ...
        if running_before is True:
            stop_status, _ = docker_call(
                self.server.docker_socket,
                "POST",
                f"/containers/{quote(self.server.target_container, safe='')}/stop"
                f"?t={self.server.stop_timeout_s}",
            )
        deadline = time.monotonic() + self.server.stop_timeout_s + 2.0
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(0.05)
        stopped = (
            running_before is True
            and stop_status == 204
            and running_after is False
        )
        result = {
            ...
            "docker_stop_status": stop_status,
            "running_after": running_after,
            "graceful_stop_confirmed": stopped,
            "stop_confirmed_unix_ns": time.time_ns() if stopped else None,
        }
        print(json.dumps(result, sort_keys=True), flush=True)
        self._reply(200 if stopped else 503, result)
...
def main() -> int:
    ...
    parser.add_argument("--stop-timeout-s", type=int, default=10)
    ...
    if args.stop_timeout_s <= 0:
        parser.error("stop timeout must be positive")
    server = SwitchoverServer((args.host, args.port), SwitchoverHandler)
    ...
    server.stop_timeout_s = args.stop_timeout_s
