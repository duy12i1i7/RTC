"""Move a live redundancy budget between robots after path quality reverses."""
...
SCHEMA_VERSION = "fleetrmw.rmw_router_multi_robot_qoe_protection_migration_probe.v1"
...
def main() -> int:
    ...
    parser.add_argument("--topic-prefix", default=f"{DEFAULT_TOPIC_PREFIX}_qoe_migration")
    ...
    parser.add_argument(
        ...
        default=(
            "results_rmw_socket/"
            "docker_router_multi_robot_qoe_protection_migration_probe_summary.json"
        ),
    )
    ...
    if args.json:
        ...
        print("fleetrmw-router-multi-robot-qoe-protection-migration-probe")
        print(f"  status: {summary['status']}")
        print(f"  epoch_path_plans: {summary.get('epoch_path_plans')}")
