#!/usr/bin/env python3
"""Validate scoped active/passive QUIC gateway durability across three instances."""
...
SCHEMA_VERSION = "fleetrmw.docker_quic_durable_failover_probe.v1"
PROBE_SCHEMA_VERSION = "fleetrmw.quic_durable_failover_probe.v1"


PROBE_EXPECTATIONS = {
    "publish": (1, 1, 3, 2),
    "resume-prefix": (2, 2, 3, 1),
    "resume-tail": (1, 1, 1, 0),
}


def probe_ok(row: dict[str, Any], mode: str) -> bool:
    connections, handshakes, streams, reuse = PROBE_EXPECTATIONS[mode]
    return (
        row.get("schema_version") == PROBE_SCHEMA_VERSION
        and row.get("status") == "ok"
        and row.get("mode") == mode
        and row.get("connections_created") == connections
        and row.get("handshakes_completed") == handshakes
        and row.get("streams_opened") == streams
        and row.get("connection_reuse_count") == reuse
...
def service_ok(row: dict[str, Any], mode: str) -> bool:
    ...
    common = (
        ...
        and durable.get("retained_frame_count") == 3
        and durable.get("dedup_key_count") == 3
    )
    if mode == "publish":
        return (
            common
            and metrics.get("requests_total") == 3
            ...
            and metrics.get("durable_frame_commits") == 3
            and metrics.get("durable_cursor_commits") == 0
            and metrics.get("recovered_frames") == 0
            and metrics.get("recovered_dedup_keys") == 0
            and metrics.get("recovered_consumers") == 0
    ...
    if mode == "resume-prefix":
        return (
            ...
            and metrics.get("durable_cursor_commits") == 2
            and metrics.get("recovered_frames") == 3
            and metrics.get("recovered_dedup_keys") == 3
            and metrics.get("recovered_consumers") == 0
    ...
    return (
        ...
        and metrics.get("get_requests") == 1
        and metrics.get("accepted_frames") == 0
        and metrics.get("duplicate_frames") == 0
        and metrics.get("dequeued_frames") == 1
        and metrics.get("durable_frame_commits") == 0
        and metrics.get("durable_cursor_commits") == 1
        and metrics.get("recovered_frames") == 3
        and metrics.get("recovered_dedup_keys") == 3
        and metrics.get("recovered_consumers") == 1
        and metrics.get("consumer_count") == 1
        and durable.get("consumer_cursor_count") == 1
        and transport.get("connections_created") == 1
        and transport.get("h3_sessions_negotiated") == 1
...
def run_phase(
    *,
    ...
    service_name = f"fleetrmw-durable-service-{suffix}"
    ...
    try:
        if ready:
            client_command = (
                ...
                "export FLEETQOX_RMW_QUIC_GATEWAY=fleetqox-admission-gateway:4501 && "
                ...
                f"fleetrmw_quic_durable_failover_probe {mode}"
            )
            client = run(
                [
                    "docker",
                    ...
                    "--rm",
                    "--name",
                    f"fleetrmw-durable-client-{suffix}",
                    "--network",
                    network,
                    "--cap-add",
                    "NET_ADMIN",
    ...
    expected_client_qlogs = PROBE_EXPECTATIONS[mode][0]
    netem_ok = "qdisc netem" in service_logs and "qdisc netem" in client.stdout
    qlog_ok = (
        len(service_files) >= 1
        and len(client_files) == expected_client_qlogs
...
def run_case(
    *,
    ...
    phases = []
    for mode in ("publish", "resume-prefix", "resume-tail"):
        phase = run_phase(
            root=root,
            image=image,
            network=network,
        ...
        if phase["status"] != "ok":
            break
    ok = len(phases) == 3 and all(phase["status"] == "ok" for phase in phases)
    return {
        "index": index,
        "status": "ok" if ok else "failed",
...
def run_probe(
    *, root: Path, image: str, iterations: int, keep_temp: bool
    ...
    temp_root = root / f".tmp_fleetrmw_quic_durable_failover_{os.getpid()}"
    certs = temp_root / "certs"
    certs.mkdir(parents=True, exist_ok=True)
    build_root = "/work/.tmp_fleetrmw_quic_durable_failover_build"
    install = "/work/.tmp_fleetrmw_quic_durable_failover_install"
    log_root = "/work/.tmp_fleetrmw_quic_durable_failover_log"
    cert_result = run(
        [
            "docker",
            "run",
    ...
    network = f"fleetrmw-durable-failover-net-{os.getpid()}"
    ...
    return {
        ...
        "active_passive_frame_recovery_claim": status == "ok",
        "failover_dedup_recovery_claim": status == "ok",
        "failover_consumer_cursor_resume_claim": status == "ok",
        "sequential_gateway_instance_failover_claim": status == "ok",
        "active_active_consensus_claim": False,
        "cluster_wide_admission_state_claim": False,
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        default="results_rmw_socket/docker_quic_durable_failover_probe_summary.json",
    )
    ...
    print("fleetrmw-quic-durable-failover-probe")
