"""Repeat remote offered/requested deadline-missed events under Docker/netem."""
...
def endpoint_ok(probe: dict[str, Any], claim: str) -> bool:
    return (
        probe.get("status") == "ok"
        and probe.get(claim) is True
        and probe.get("initial_not_ready") is True
        and probe.get("cleared_not_ready") is True
        and int(probe.get("total_count", 0)) >= 1
        and int(probe.get("total_count_change", 0)) >= 1
        and int(probe.get("callback_events", 0)) >= 1
...
def run_case(
    *, root: Path, image: str, install: str, network: str, index: int
    ...
    observer = f"fleetrmw-remote-deadline-observer-{os.getpid()}-{index}"
    advertiser = f"fleetrmw-remote-deadline-advertiser-{os.getpid()}-{index}"
    binary = (
        f"{install}/rmw_fleetqox_cpp/lib/rmw_fleetqox_cpp/"
        "fleetrmw_remote_deadline_event_probe"
    )
    common_env = (
    ...
    observer_command = (
        ...
        f"{common_env}FLEETQOX_RMW_BIND=0.0.0.0:48720 "
    ...
    try:
        if ready:
            advertiser_command = (
                f"source /opt/ros/jazzy/setup.bash && source {install}/setup.bash && "
                "tc qdisc replace dev eth0 root netem delay 5ms 1ms && "
                f"{common_env}FLEETQOX_RMW_BIND=0.0.0.0:48721 "
                f"FLEETQOX_RMW_PEERS={observer}:48720 "
    ...
    ok = (
        ...
        and observer_probe.get("sample_taken") is True
        and endpoint_ok(observer_probe, "requested_deadline_missed_event_claim")
        and endpoint_ok(advertiser_probe, "offered_deadline_missed_event_claim")
    )
    return {
        "index": index,
...
def run_probe(
    *, root: Path, image: str, iterations: int, keep_temp: bool
    ...
    build_root = "/work/.tmp_fleetrmw_remote_deadline_event"
    ...
    network = f"fleetrmw-remote-deadline-net-{os.getpid()}"
    ...
    return {
        ...
        "deadline_ms": 100,
        "remote_offered_deadline_missed_event_claim": ok,
        "remote_requested_deadline_missed_event_claim": ok,
        "remote_deadline_missed_event_repeated_claim": ok and run_count >= 5,
        "offered_total_count": last_advertiser.get("total_count"),
        "offered_callback_events": last_advertiser.get("callback_events"),
        "requested_total_count": last_observer.get("total_count"),
        "requested_callback_events": last_observer.get("callback_events"),
        "clean_teardown": all(
...
def main() -> int:
    ...
    parser.add_argument(
        "--summary-json",
        default=(
            "results_rmw_socket/docker_remote_deadline_event_probe_summary.json"
        ),
    )
